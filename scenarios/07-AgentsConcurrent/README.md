# Scenario 07 - Concurrent Agent Orchestration with Microsoft Agent Framework

## Description

This scenario demonstrates concurrent agent orchestration using the [Microsoft Agent Framework (MAF)](https://github.com/microsoft/agents). The goal is to showcase how multiple agents can work in parallel to analyze user queries and provide valuable insights for future analysis.
...
- **Concurrent Agent Execution:** Utilizes the Microsoft Agent Framework to orchestrate multiple agents running in parallel.
...
The solution is in the `./src` folder, the main solution is **[eShopLite-07-AgentsConcurrent.slnx](./src/eShopLite-07-AgentsConcurrent.slnx)**.
...
```bash
aspire secret set Parameters:AzureOpenAIEndpoint "https://<your-resource>.openai.azure.com/" --apphost scenarios/07-AgentsConcurrent/src/eShopAppHost/eShopAppHost.csproj
aspire secret set Parameters:AzureOpenAIApiKey "<your-api-key>" --apphost scenarios/07-AgentsConcurrent/src/eShopAppHost/eShopAppHost.csproj
aspire secret set Parameters:AzureOpenAIDeploymentName "gpt-4.1-mini" --apphost scenarios/07-AgentsConcurrent/src/eShopAppHost/eShopAppHost.csproj
aspire secret set Parameters:AzureOpenAIEmbeddingsDeploymentName "text-embedding-ada-002" --apphost scenarios/07-AgentsConcurrent/src/eShopAppHost/eShopAppHost.csproj
```
